    ...
    <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
      ...
      <section className="rounded-2xl border border-line bg-panel p-5">
        ...
        <div className="space-y-4">
          ...
          <Field label="Time horizon" value={years} onChange={(v) => setYears(Math.round(v))} min={1} max={40} step={1} display={`${years} yr`} />
          <Field label="Simulations (per scenario)" value={nSims} onChange={(v) => setNSims(Math.round(v))} min={1000} max={20_000} step={1000} display={nSims.toLocaleString()} />
        </div>
